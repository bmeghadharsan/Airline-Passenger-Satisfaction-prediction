...
pickle_in3= open("data/catBoost.pkl","rb")
catb=pickle.load(pickle_in3)
pickle_in4= open("data/KNN.pkl","rb")
knn=pickle.load(pickle_in4)
pickle_in5= open("data/GNB.pkl","rb")
gnb=pickle.load(pickle_in5)
pickle_in6= open("data/lgb1.pkl","rb")
lgb=pickle.load(pickle_in6)
pickle_in7= open("data/xgb1.pkl","rb")
xgb=pickle.load(pickle_in7)
...
def lottie():
    lottie_hello = load_lottieurl("https://assets7.lottiefiles.com/packages/lf20_DXljHQsLLA.json")
...
def predict_note_authentication(opt,pred):
    ...
    if opt=='Cat Boost':
        prediction = catb.predict([pred])
    if opt=='KNN':
        prediction=knn.predict([pred])
    if opt=="GNB":
        prediction=gnb.predict([pred])
    if opt=="LG Boost":
        prediction=lgb.predict([pred])
    if opt=="XG Boost":
...
if choice=='Models':
    option = st.selectbox('Choose the Model: ',
        ('Select Model','Decision Tree', 'Logistic Regression', 'Random Forest','KNN','GNB','Cat Boost','LG Boost','XG Boost'))
    ...
    lottie("https://assets2.lottiefiles.com/packages/lf20_wTUuPtXs3u.json",200)
        
if choice=='Vanakkam':
    #from PIL import Image
    #image = Image.open('images/ml.jpg')
    #st.image(image, caption='Let us have some Fun :)')
    lottie("https://assets7.lottiefiles.com/packages/lf20_DXljHQsLLA.json",450)
        
#Plots
if choice=='Plots':
    lottie('https://assets6.lottiefiles.com/packages/lf20_q5qeoo3q.json',500)
